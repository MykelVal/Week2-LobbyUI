...
namespace Photon.Pun.Demo.PunBasics
{
	...
	public class Launcher : MonoBehaviourPunCallbacks
    {
		...
		[SerializeField] private List<PlayerContainer> _playerContainerList;
		...
		private void UpdateRoomStatistics()
		{
            ...
            foreach (var item in _playerContainerList)
            {
				Destroy(item.gameObject);
            }
			_playerContainerList.Clear();

            foreach (var player in PhotonNetwork.CurrentRoom.Players)
            {
				PlayerContainer newPlayerContainer = Instantiate(_playerContainerPrefab, _playerListParent);
				newPlayerContainer.UpdatePlayerInfo(player.Value, player.Key);
				_playerContainerList.Add(newPlayerContainer);
                //_roomStatistics.text += $"\n #{player.Key} | {player.Value.NickName} | {player.Value.ActorNumber} | {player.Value.UserId} ";
            }
        }

		private void TransferHost(Player newMaster)
        {
